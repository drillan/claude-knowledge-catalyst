...
echo "─────────────────────────────────────────────"
echo "$ cd ../obsidian_vault && ckc migrate --preview"
echo ""
cd "../obsidian_vault"
run_ckc migrate --preview
cd "../ckc_project"
...
echo "─────────────────────────────────────────────────────────"
echo "$ cd ../obsidian_vault && ckc migrate --auto-apply"
echo ""
cd "../obsidian_vault"
run_ckc migrate --auto-apply
cd "../ckc_project"
...
for dir in "_system" "inbox" "active" "archive" "knowledge"; do
    if [ -d "../enhanced_ckc_vault/$dir" ]; then
        echo "📁 $dir/:"
        find "../enhanced_ckc_vault/$dir" -maxdepth 1 -name "*.md" -o -name "*.yaml" 2>/dev/null | head -3 | while read -r file; do
            echo "    📄 $(basename "$file")"
        done || echo "  (no files or empty)"
        echo ""
    fi
done
...
if [ -d "$template_dir" ]; then
    ...
    find "$template_dir" -name "*.md" 2>/dev/null | while read -r template_file; do
