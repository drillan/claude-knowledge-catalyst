...
class ObsidianVaultManager:
    ...
    def sync_file(self, source_path: Path, project_name: Optional[str] = None) -> bool:
        ...
        try:
            ...
            target_path = self._determine_target_path(metadata, source_path, project_name)
    ...
    def sync_directory(self, source_dir: Path, project_name: Optional[str] = None) -> Dict[str, bool]:
    ...
    def _determine_target_path(
        ...
        if "prompt" in metadata.tags:
            if metadata.status == "production" and metadata.success_rate and metadata.success_rate > 80:
                base_path = "02_Knowledge_Base/Prompts/Best_Practices"
            elif "improvement" in str(source_path).lower() or "version" in metadata.dict():
    ...
    def _enhance_content_for_obsidian(self, source_path: Path, metadata: KnowledgeMetadata) -> str:
        ...
        if metadata.related_projects:
            links = " | ".join(f"[[{project}]]" for project in metadata.related_projects)
        ...
        enhanced_content = enhanced_frontmatter + "\n" + "\n".join(enhancements) + "\n" + content_without_frontmatter
...
class ObsidianSyncManager:
    ...
    def sync_single_file(self, file_path: Path, project_name: Optional[str] = None) -> bool:
        """Sync a single file.
    ...
    def sync_claude_directory(self, claude_dir: Path, project_name: Optional[str] = None) -> Dict[str, bool]:
    ...
    def get_sync_status(self) -> Dict[str, any]:
        ...
        return {
            ...
            "vault_stats": self.vault_manager.get_vault_stats() if self.sync_target.path.exists() else {},
        }