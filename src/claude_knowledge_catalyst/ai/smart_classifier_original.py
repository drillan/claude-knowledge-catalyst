...
class SmartContentClassifier:
    ...
    def _classify_technology(self, content: str) -> list[ClassificationResult]:
        ...
        for tech, patterns in self.tech_patterns.items():
            ...
            keyword_matches = sum(
                1 for keyword in patterns["keywords"] if keyword in content
            )
    ...
    def _classify_complexity(
        ...
        for complexity, indicators in self.complexity_indicators.items():
            ...
            if (
                complexity == "beginner"
                and content_length < indicators["length_threshold"]
            ):
                ...
                if (
                    indicators["length_threshold"] * 0.5
                    < content_length
                    < indicators["length_threshold"] * 2
                ):
    ...
    def _classify_confidence(self, content: str) -> list[ClassificationResult]:
        ...
        high_matches = sum(
            1 for pattern in high_confidence_patterns if pattern in content
        )
        low_matches = sum(
            1 for pattern in low_confidence_patterns if pattern in content
        )
    ...
    ) -> dict[Path, list[ClassificationResult]]:
            ...
            if classifications:
                ...
                for classification in classifications:
                    ...
                    if classification.tag_type == "tech":
                        ...
                        summary["top_technologies"][tech] = (
                            summary["top_technologies"].get(tech, 0) + 1
                        )
    ...
    def _enhance_classification_with_yake(
        ...
        for result in enhanced_results:
            if any(
                keyword in yake_keywords for keyword in [result.suggested_value.lower()]
            ):
                result.confidence = min(0.95, result.confidence + 0.1)
                result.evidence.append(
                    f"YAKE keyword match: '{result.suggested_value}'"
                )
        ...
        for tech, keywords in tech_keywords.items():
            if any(kw in yake_keywords for kw in keywords) and not any(
                r.suggested_value == tech
