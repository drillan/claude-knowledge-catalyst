...
class PatternLoader:
    ...
    def _validate_patterns(self, patterns: Dict[str, Any], filename: str) -> None:
        ...
        if not isinstance(patterns, dict):
            raise ValueError(f"Invalid pattern file {filename}: root must be a dictionary")
        
        for pattern_name, pattern_data in patterns.items():
            if not isinstance(pattern_data, dict):
                raise ValueError(f"Invalid pattern {pattern_name} in {filename}: must be a dictionary")
            ...
            for level in required_levels:
                if level not in pattern_data:
                    raise ValueError(f"Pattern {pattern_name} in {filename} missing required level: {level}")
                
                if not isinstance(pattern_data[level], list):
                    raise ValueError(f"Pattern {pattern_name}.{level} in {filename} must be a list")
