...
def cli(ctx: click.Context, config: Optional[Path]) -> None:
    """Claude Knowledge Catalyst - Sync your Claude Code insights with knowledge management tools."""
...
@click.pass_context
def sync_add(ctx: click.Context, name: str, type: str, path: Path, disabled: bool) -> None:
    ...
    if type == "obsidian":
        ...
        if obsidian_manager.initialize():
            ...
            console.print(f"[yellow]![/yellow] Failed to initialize Obsidian vault")
...
@click.pass_context
def sync_run(ctx: click.Context, target: Optional[str], project: Optional[str]) -> None:
    ...
    for sync_target in targets_to_sync:
        ...
        try:
            if sync_target.type == "obsidian":
                ...
                console.print(f"[yellow]![/yellow] Sync type '{sync_target.type}' not yet implemented")
...
def status(ctx: click.Context) -> None:
    ...
    if not config.sync_targets:
        ...
        for target in config.sync_targets:
            status = "[green]✓[/green]" if target.enabled and target.path.exists() else "[red]✗[/red]"
...
def analyze(ctx: click.Context, file_path: Path) -> None:
    ...
    try:
        ...
        table.add_row("Success Rate", f"{metadata.success_rate}%" if metadata.success_rate else "N/A")
        ...
        if metadata.related_projects:
            console.print(f"\n[blue]Related Projects:[/blue] {', '.join(metadata.related_projects)}")
