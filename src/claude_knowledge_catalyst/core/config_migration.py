...
from .hybrid_config import NumberingSystem
...
class ConfigMigrationManager:
    ...
    def _migrate_config(
        self, raw_data: dict, from_version: str, config_path: Path
    ) -> CKCConfig:
...
class ConfigCompatibilityValidator:
    ...
    def validate_v1_compatibility(self, config: CKCConfig) -> bool:
        ...
        required_v1_fields = ["project_name", "sync_targets", "auto_sync"]
    ...
    def get_compatibility_issues(self, config: CKCConfig) -> list[str]:
        ...
        for target in config.sync_targets:
            if target.type == "obsidian":
                ...
                if (
                    not config.hybrid_structure.legacy_support
