...
class TestEssentialCLICommands:
    ...
    def test_ckc_init_command(self, cli_runner, isolated_project):
        ...
        assert result.exit_code in [0, 1]  # 0 = success, 1 = expected error with guidance
    ...
    def test_ckc_status_command(self, cli_runner, isolated_project):
        ...
        assert result.exit_code in [0, 1]  # Allow for expected errors in test environment
        if result.exit_code == 0:
...
```'''
        ...
        for dir_name in expected_dirs:
            assert (vault_path / dir_name).exists(), f"Should create {dir_name} directory"
...
Technologies: Python, FastAPI, Pydantic, async, REST API'''
        ...
        assert metadata.type in ["code", "concept", "resource"], "Should detect appropriate type"
        ...
        assert any("api" in tag or "fastapi" in tag for tag in tech_tags), "Should detect API technology"
